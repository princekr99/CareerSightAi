...
export default function Signup({ onSignup, theme }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Registration failed. Verify backend service is online.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        <input
          ...
          className={`w-full rounded-xl border px-4 py-2.5 text-sm transition focus:outline-none focus:ring-2 focus:ring-violet-500/50 ${
            isDark
      ...
      <button
        type="submit"
        disabled={loading}
        className={`w-full rounded-xl py-3 text-sm font-bold shadow-md transition hover:scale-[1.01] active:scale-[0.99] flex items-center justify-center gap-2 mt-2 ${
          isDark
